...
namespace GroceryList
{
	...
	public class AddItemActivity : Activity
	{
		...
		void OnSaveClick(object sender, EventArgs e)
		{
            ...
            //load values in the intent bundle object to send to the source Activity
            intent.PutExtra("ItemName", name);
            intent.PutExtra("ItemCount", count);
            //set the result and the data to be passed back to the source Activity
            SetResult(Result.Ok, intent);
            //end current Activity and return the source Activity
            Finish();
		}

		void OnCancelClick(object sender, EventArgs e)
		{
            //finsh acts as a back operation
            Finish();
		}
	}
}